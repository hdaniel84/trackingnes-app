...
import TrackingPrensasEditDialog from './TrackingPrensasEditDialog.vue';
...
const deleteItem = async () => {
  try {
    ...
    console.error('Ocorreu um erro ao eliminar o registo:', error);
  }
};
*/
</script>
  ...
  <div class="w-full">
    ...
    <div v-else class="flex flex-col gap-4 max-h-[80vh] overflow-y-auto pr-2">
      <div v-for="item in store.items" :key="item.id" 
        ...
        <div class="flex justify-between items-start gap-4" @click="openDetailDialog(item)">
          <div class="flex items-start gap-3 flex-1">
            ...
            <div class="flex flex-col w-full">
              ...
              <span class="text-xs text-surface-400 dark:text-surface-500 mt-2">
                <i class="pi pi-calendar mr-1"></i> {{ new Date(item.startTime).toLocaleString('pt-PT') }}
              </span>
              <span class="text-xs text-surface-400 dark:text-surface-500 mt-2">
    ...
    <TrackingPrensasEditDialog v-model:visible="showEditDialog" :item="selectedItem" />

    <TrackingPrensasDetailDialog v-model:visible="showDetailDialog" :item="selectedItem" />
    <!-- Diálogo de confirmación NO USADO en este caso 
    <ConfirmDialog
      v-model:visible="showConfirmDialog"
      message="Tem a certeza de que pretende eliminar este registo?"
      @confirm="deleteItem"
    />-->
  </div>
</template>
