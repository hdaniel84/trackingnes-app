    ...
    <Dialog :visible="props.visible" @update:visible="emit('update:visible', $event)" modal
        ...
        <div v-if="props.item" class="flex flex-col gap-6">
            ...
            <div v-if="props.item.trackingSourceId" class="mt-2">
                <div
                    class="bg-cyan-50 dark:bg-cyan-900/20 border border-cyan-200 dark:border-cyan-800 rounded-lg p-3 flex items-center gap-4 relative overflow-hidden">
                    <div class="absolute -right-4 -top-4 text-cyan-100 dark:text-cyan-800/30 text-6xl">
                        <i class="pi pi-link"></i>
                    </div>

                    <div
                        class="z-10 w-10 h-10 rounded-full bg-white dark:bg-cyan-900 border border-cyan-100 dark:border-cyan-700 flex items-center justify-center text-cyan-600 dark:text-cyan-400 shadow-sm">
                        <i class="pi pi-link text-lg"></i>
                    </div>

                    <div class="z-10 flex flex-col">
                        <span
                            class="text-[10px] uppercase font-bold text-cyan-600 dark:text-cyan-400 tracking-wider">Origem
                            (Fase Anterior)</span>
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            <span
                                class="text-xs text-surface-500 bg-white dark:bg-surface-800 px-1.5 py-0.5 rounded border border-surface-200 dark:border-surface-700 font-mono">
            ...
            <div v-if="props.item.parameters && props.item.parameters.length > 0">
                <Divider align="left">
                    <div
                        class="inline-flex items-center gap-2 px-2 py-1 rounded-full bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-300 text-xs font-bold uppercase tracking-wide">
                        <i class="pi pi-link"></i> Parâmetros
                    </div>
                </Divider>
                    ...
                    <div v-for="(param, index) in props.item.parameters" :key="index"
                        ...
                        <div class="absolute top-0 right-0 p-1">
                            ...
                            <i v-else-if="param.valueNumber !== null"
                                class="pi pi-link text-xs text-surface-300"></i>
